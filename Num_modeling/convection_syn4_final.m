%file:   convection_syn4.m

%author:  Dominik A. Kardell, modified from Evan J. Ramos
%date:    15 Aug 2020

%Convection script for "averaged" crust with line1A thickness sediment layer

clear, clc, close all

load phys_props_syn4.mat %Import physical property grids
load layer2A.mat %Import indeces of extrusive layer for residence time calculation
...
%     end
% end
% T_bot = (273.15+63+(cr*dyn/1000-2)*31)'; %355.15;         %[K] basal temperature. Linearly interpolating between 54 C at 2 km and 82 C at 3 km
...
ocean_thick_vector = linspace(mean(ocean_thick_vector)+0.5,mean(ocean_thick_vector)-0.5,Nx)'; %3412,3318
...
if mod(count,plotint) == 0

figure(4)
    set(gcf,'units','normalized','outerposition',[0.5 0 0.2 0.5]) %[0 0 1 0.9]
    subplot(3,1,1)
...
figure(5)
set(gcf,'units','normalized','outerposition',[0.4 0 0.1 0.5]) %[0 0 1 0.9]
    subplot(5,1,1)
    plot(T_top)
