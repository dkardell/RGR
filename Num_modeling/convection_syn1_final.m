%file:   convection_syn1.m

%author:  Dominik A. Kardell, modified from Evan J. Ramos
%date:    15 Aug 2020

%Convection script for 3-layer model with constant thickness sediment layer

clear, clc, close all

load phys_props_syn1.mat %Import physical property grids
...
figure(4)
    set(gcf,'units','normalized','outerposition',[0.2 0.5 0.2 0.5]) %[0 0 1 0.9]
    subplot(3,1,1)
...
figure(5)
set(gcf,'units','normalized','outerposition',[0.1 0.5 0.1 0.5]) %[0 0 1 0.9]
    subplot(5,1,1)
    plot(T_top)
...
% med_age_2A = median(ages(K_syn == 1e-13)/yrs2s);
% ave_age_2A = mean(ages(K_syn == 1e-13)/yrs2s);
